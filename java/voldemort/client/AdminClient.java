...
public class AdminClient {
    ...
    public AbstractIterator<Pair<ByteArray, Versioned<byte[]>>> requestGetPartitionsAsStream(int nodeId,
                                                                                             String storeName,
    ...
    public void requestPutEntriesAsStream(int nodeId,
        ...
        try {
            ...
            while(entryIterator.hasNext()) {
                ...
                // System.out.println("Client :: put(Node:" + nodeId + " store:"
                // + storeName
    ...
    public void refreshServices(int nodeId) {
        ...
        try {
            DataOutputStream outputStream = sands.getOutputStream();
            outputStream.writeByte(VoldemortOpCode.REFRESH_SERVICES_OP_CODE);
            outputStream.flush();
    ...
    public void changeStateAndRefresh(int nodeId, SERVER_STATE state) {
        Cluster currentCluster = metadataStore.getCluster();
        Node node = currentCluster.getNodeById(nodeId);
        SocketDestination destination = new SocketDestination(node.getHost(), node.getAdminPort());
    ...
    public void pipeGetAndPutStreams(int donorNodeId,
        ...
        requestPutEntriesAsStream(stealerNodeId,
                                  storeName,
                                  requestGetPartitionsAsStream(donorNodeId, storeName, stealList));
    }
